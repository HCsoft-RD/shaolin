...
class PanelScatter(Shaolin):
    ...
    def init_mapped_params_sel(self):
        ...
        for atr in self.marker_params:
            a = getattr(self.marker_map_sel, atr)
            a.ax1.observe(self.trigger_update, names='value')
            a.ax2.observe(self.trigger_update, names='value')
            a.active.observe(self.update_active, names='value')
    ...
    def select_tooltip_data(self, strf='%d-%m-%y %H:%M'):
        ...
        cols = dfx.columns
        dfx.columns = [x+'_tipx' for x in dfx.columns]
        dfy.columns = [y+'_tipy' for y in dfy.columns]

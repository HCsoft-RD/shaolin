...
class VPGraphPlot(Shaolin):
    ...
    def __init__(self, gc,
        ...
        super(VPGraphPlot, self).__init__()
        self.init_widget()
    ...
    def init_plot(self,
        ...
        self.canvas = vp.canvas(height=height,
                       width=width,
                       background=background,
                       center=vp.vector(+0.9,+0.5,0))
        
        #self.canvas.center=vp.vector(+0.9,+0.5,0)
        self.create_vpython_nodes()
        self.create_vpython_edges()
        #self.init_default_active()

    def create_vpython_nodes(self):
        s = self.node_source
        for node in self.gc.G.nodes():
            pvec = self.array_to_vector(s.loc[node,['x','y']].values)            
            c = s.loc[node,'fill_color']            
            r = s.loc[node,'size']/600.0
            alpha = s.loc[node,'fill_alpha']
            #print('NODE',node,'COLOR',c,'RADIUS',r,alpha)
            self.nodes.loc['node',node] = vp.sphere(pos=pvec, 
                                   color=self.HTMLColorToVector(c),
                                   radius=r,
                                   opacity = alpha,
            ...
            self.labels.loc['node',node] = vp.label(pos=pvec,
                                    text=s.loc[node,'label'],
                                    space=r,
                                    #xoffset=self.G.node[node]['r'],
                                    yoffset=r,
                                    line=True,
                                    box=False,
                                    height=7,
        ...
        self.nodes = self.nodes.fillna(vp.sphere(visible=False,
                                    opacity = 0.9,make_trail = self.make_trail,
                                   trail_type="points",canvas=self.canvas))
        self.labels = self.labels.fillna(vp.label(visible=False,
                                   line=True,
                                   box=False,
                                   height=10,canvas=self.canvas))
        
    
    def create_vpython_edges(self, pos_const=1):
        #nodes = self.gc.G.nodes()
        #e_ix = pd.MultiIndex.from_product([nodes,nodes]).values
        self.edges = pd.DataFrame(index=self.gc.G.nodes(),columns=self.gc.G.nodes())
        s = self.edge_source.copy()
        #print(s.columns)
        #s[s=='NaN'] = np.nan
        #s = s.fillna(method='ffill').fillna(method='bfill')
        #self.edge_source = s

        for u,v in self.edge_source.index:
            x0 = s.loc[u,v][['x0','y0']].values
            x1 = s.loc[u,v][['x1','y1']].values
            posvec = self.array_to_vector(x0)
            axis = self.array_to_vector(x1-x0)
            co = self.HTMLColorToVector(s.ix[u,v]['line_color'])
            r = s.ix[u,v]['line_width']
            #print(s,u0,'MIAAAAAAU',co,'RRR',r)
            c = vp.cylinder(pos=posvec, axis=axis,
                         radius=r/240.0,
                         color=co,
                        opacity=s.ix[u,v]['line_alpha'],display=self.canvas)#, radius=1/pos_const*2.0)
            self.edges.loc[u,v] = c
        #.fillna(vp.cylinder(visible=False,opacity=0.3,display=self.canvas))
        #return ed
    
    def update_nodes(self):
        s = self.node_source
        for node in self.gc.G.nodes():
            pvec=self.array_to_vector(s.loc[node,['x','y']])
            self.nodes.loc['node',node].pos = pvec
            self.nodes.loc['node',node].color= self.HTMLColorToVector(s.loc[node,'fill_color'])
            self.nodes.loc['node',node].opacity= s.loc[node,'fill_alpha']
            self.nodes.loc['node',node].radius = s.loc[node,'size']/600
            self.nodes.loc['node',node].visible = True
            
            self.labels.loc['node',node].pos = pvec
            self.labels.loc['node',node].text = s.loc[node,'label']
            self.labels.loc['node',node].space = s.loc[node,'size']/600
            self.labels.loc['node',node].yoffset = s.loc[node,'size']
            self.labels.loc['node',node].visible = True
    
    def new_edge(self,u,v):
        s = self.edge_source.copy()
        df = self.edge_source.loc[u,v]
        x0 = s.loc[u,v][['x0','y0']].values
        x1 = s.loc[u,v][['x1','y1']].values
        posvec = self.array_to_vector(x0)
        axis = self.array_to_vector(x1-x0)
        co = self.HTMLColorToVector(df['line_color'])
        r = df['line_width']/240.0
    ...
    def update_edges(self):
        ...
        for u,v in s.index:
            ...
            posvec = self.array_to_vector(x0)
            axis = self.array_to_vector(x1-x0)
            if not  type(self.edges.loc[u,v]) is vp.vpython.cylinder:
                self.new_edge(u,v)                
            else:
                self.edges.loc[u,v].pos = posvec
                self.edges.loc[u,v].axis = axis
                self.edges.loc[u,v].radius =s.loc[u,v]['line_width']/240.0
                self.edges.loc[u,v].color = self.HTMLColorToVector(s.loc[u,v]['line_color'])
                self.edges.loc[u,v].opacity=s.loc[u,v]['line_alpha']
                self.edges.loc[u,v].visible = True

    def HTMLColorToVector(self,colorstring):
        """ convert #RRGGBB to an (R, G, B) tuple """
        try :
            rgb =  wc.html5_parse_simple_color(colorstring)
        except:
            rgb = wc.name_to_rgb(colorstring)
        return self.array_to_vector(np.array(rgb)/255.0)
            
    def array_to_vector(self,x):
        if len(x) >= 3:
            return vp.vector(x[0],x[1],x[2])
        if len(x) == 2:
            return vp.vector(x[0], x[1], 0.)
        if len(x) == 1:
            return vp.vector(x[0], 0., 0.)
    
    
    def init_default_active(self):
        self.node_sel.fill_color.active.value = True
        self.update()
        
    def update_make_trail(self,trail=True):
        def one_trail(x,trail=trail):
            x.make_trail=trail
            return x
        self.nodes.applymap(one_trail)
    
    
    def clear_all_edges(self):
        def invisible(x):
            if type(x) is vp.vpython.cylinder:
                x.visible=False
            return x
        #self.nodes = self.nodes.applymap(invisible)
        #self.labels = self.labels.applymap(invisible)
        self.edges =self.edges.applymap(invisible)
        
              
        
    def clear_display(self):
        nodes = self.tsg.gc.ix[self.i].G.nodes()
        max_edges = pd.MultiIndex.from_product([nodes,nodes]).values
        e = self.tsg.gc.ix[self.i].G.edges()
        eM = set(max_edges)
        eG = set(e)
        for i,j in list(eM-eG):
            if type(self.edges.loc[i,j]) is vp.vpython.cylinder:
                #print(self.edges.loc[i,j])
                self.edges.loc[i,j].visible = False  
